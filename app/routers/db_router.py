...
from app.models.course_model import CourseCreateParams, CourseUpdateParams, CourseCreateResp, CourseStudentQueryResp, \
    CourseStudentResp, CourseResp, CourseQueryResp, CourseSelectParams
from app.models.generic_error import GenericError, err_course_cap_conflict, err_course_not_exist, \
    err_course_id_conflict, err_course_id_full, err_teacher_not_exist
...
# todo text的in查询参数不能直接这么用，想想别的办法

@router.post('/courses/select', status_code=204)
async def select_course(shard_conn: ShardConn, p: CourseSelectParams):
    pass


@router.post('/courses/deselect', status_code=204)
async def deselect_course(shard_conn: ShardConn, p: CourseSelectParams):
    pass
